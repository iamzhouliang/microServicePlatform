...
package com.microservice.platform.ai.core.provider.graph.neo4j;
...
import com.microservice.platform.ai.core.provider.graph.GraphRetriever;
        ...
        try {
            ...
            try (Session session = graphStore.getDriver().session()) {
                ...
                log.info("向量检索完成: question={}, resultsCount={}",
                        question.substring(0, Math.min(AiServiceConstants.SHORT_LOG_TRUNCATE_LENGTH, question.length())), results.size());
            }
        } catch (Exception e) {
    ...
    public List<String> retrieveByVector(String knowledgeBaseId, String question,
        ...
        if (anchorEntities.isEmpty()) {
            log.info("向量检索未找到匹配实体: knowledgeBaseId={}, question={}",
                    knowledgeBaseId, question.substring(0, Math.min(AiServiceConstants.SHORT_LOG_TRUNCATE_LENGTH, question.length())));
            return Collections.emptyList();
        }
