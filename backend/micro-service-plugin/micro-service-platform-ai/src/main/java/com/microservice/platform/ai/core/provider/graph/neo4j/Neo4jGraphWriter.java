...
public class Neo4jGraphWriter {
    ...
    private List<Float> generateEmbedding(String text) {
        ...
        try {
            ...
            log.warn("生成向量嵌入失败: text={}, error={}",
                    text.substring(0, Math.min(AiServiceConstants.SHORT_LOG_TRUNCATE_LENGTH, text.length())), e.getMessage());
            return null;
        }
    }
     ...
     * 逐个创建关系，每个关系使用其真实类型
     *
     * @param relationships relationships 参数
     * @param session session 参数
     */
