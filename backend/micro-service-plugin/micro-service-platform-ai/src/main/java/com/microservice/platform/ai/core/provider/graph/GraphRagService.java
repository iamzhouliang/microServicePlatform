...
import com.microservice.platform.ai.core.provider.graph.neo4j.Neo4jGraphStore;
...
public class GraphRagService {
    ...
    /**
     * 混合检索并行线程池：有界队列 + CallerRunsPolicy 背压，应用关闭时优雅 shutdown；
     * 改为实例字段后，仅在图谱启用（本 Bean 被创建）时才分配线程资源。
     */
    private final ExecutorService hybridExecutor = new ThreadPoolExecutor(
            2, 2, 0L, TimeUnit.MILLISECONDS,
            new LinkedBlockingQueue<>(100),
            ThreadFactoryBuilder.create().setNamePrefix("ai-graph-hybrid-").build(),
    ...
    public ProcessResult processDocumentsWithEmbedding(String knowledgeBaseId,
        ...
        for (Document document : documents) {
            try {
                ...
                log.error("Failed to process document: {}", document.text().substring(0,
                        Math.min(AiServiceConstants.LOG_TRUNCATE_LENGTH, document.text().length())), e);
            }
        }
     ...
     * @param includeSource includeSource 参数
     * @param graphDocuments graphDocuments 参数
     */
    ...
    /**
     * 处理结果记录
     * @param documentsProcessed documentsProcessed 参数
    ...
    /**
     * 基于向量的图谱语义检索（可配置参数）
     * @param hopDepth hopDepth 参数
     * @param maxTriples maxTriples 参数
     * @param scoreThreshold scoreThreshold 参数
     * @param searchLimit searchLimit 参数
     * @param knowledgeBaseId 知识库标识
     * @param question 用户问题
     * @return 处理结果
     */
    ...
    @PreDestroy
    public void shutdownHybridExecutor() {
        ...
        try {
            ...
            if (mergedAnchors.isEmpty()) {
                log.info("混合检索未找到任何锚点实体: question={}",
                        question.substring(0, Math.min(AiServiceConstants.SHORT_LOG_TRUNCATE_LENGTH, question.length())));
    ...
    /**
     * 解析 JSON 数组
     * @param jsonResponse jsonResponse 参数
     * @return 处理结果
     */
