...
import com.microservice.platform.wms.inbound.domain.entity.ReceivingPlanItem;
...
import com.microservice.platform.wms.inbound.domain.req.ReceivingPlanItemReceivingReq;
import com.microservice.platform.wms.inbound.domain.req.ReceivingPlanSaveReq;
import com.microservice.platform.wms.inbound.domain.resp.ReceivingPlanDetailResp;
import com.microservice.platform.wms.inbound.domain.resp.ReceivingPlanItemPageResp;
import com.microservice.platform.wms.inbound.mapper.InventoryReceiptItemMapper;
import com.microservice.platform.wms.inbound.mapper.ReceivingPlanItemMapper;
...
import com.microservice.platform.wms.inbound.service.ReceivingPlanService;
...
public class ReceivingPlanServiceImpl extends SuperServiceImpl<ReceivingPlanMapper, ReceivingPlan> implements ReceivingPlanService {
    ...
    public IPage<ReceivingPlanItemPageResp> itemPage(Page<ReceivingPlanItem> page, Long planId) {
        ...
        // 收货数量来自入库单明细，避免计划明细表冗余维护“进行中/已完成”数量。
    ...
    public void submit(Long id) {
        ...
        // 占用月台和容器
        // TODO 如果提交时已经绑定月台或容器，需要在这里生成占用记录，撤回或完成时再释放。
    ...
    public void cancel(Long id) {
        ...
        // 返还占用的月台和容器
        // TODO 与 submit 的占用逻辑保持对称，撤回后释放待执行阶段占用的作业资源。
    }
    ...
    private Pair<ReceivingPlan, List<ReceivingPlanItem>> getReceivingPlanPari(Long id) {
        ...
        // 收货动作只允许在已提交后的计划上执行，草稿还可能继续编辑，完成/关闭后不可再生成入库单。
        if (ReceivingStatus.DRAFT == receivingPlan.getStatus()) {
    ...
    public void receiving(Long id, List<ReceivingPlanItemReceivingReq> reqList) {
        ...
        var receiptItemList = reqList.stream().map(itemReq -> {
            ...
            // 可收数量 = 计划数量 - 已生成未完成入库单数量 - 已完成入库数量，防止重复收货。
    ...
    public void allReceiving(Long id) {
        ...
        var receiptQtyMap = MapHelper.toHashMap(itemsReceiptItemMapper.selectReceiptQty(itemIdList), ReceiptQtyDTO::getPlanItemId, x -> x);
        // 一键收货只为仍有剩余可收数量的明细生成入库单，已收满的行直接跳过。
    ...
    private void addInventoryReceipt(ReceivingPlan receivingPlan, List<InventoryReceiptItemSaveReq> receiptItemList) {
        // 收货动作不直接改库存，先生成草稿入库单，由入库单确认环节完成上架和计划完成判断。
