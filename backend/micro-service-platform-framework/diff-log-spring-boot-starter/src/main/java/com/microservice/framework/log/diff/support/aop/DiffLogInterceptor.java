package com.microservice.framework.log.diff.support.aop;
...
import com.microservice.framework.log.diff.IDiffItemsToLogContentService;
import com.microservice.framework.log.diff.core.context.DiffLogContext;
import com.microservice.framework.log.diff.domain.DiffLogInfo;
import com.microservice.framework.log.diff.domain.DiffLogOps;
import com.microservice.framework.log.diff.domain.FieldChange;
import com.microservice.framework.log.diff.domain.MethodExecute;
import com.microservice.framework.log.diff.service.IDiffLogPerformanceMonitor;
import com.microservice.framework.log.diff.service.IDiffLogService;
import com.microservice.framework.log.diff.service.IFunctionService;
import com.microservice.framework.log.diff.service.impl.DiffParseFunction;
import com.microservice.framework.log.diff.support.parse.DiffLogFunctionParser;
import com.microservice.framework.log.diff.support.parse.DiffLogValueParser;
...
import static com.microservice.framework.log.diff.service.IDiffLogPerformanceMonitor.*;
...
public class DiffLogInterceptor extends DiffLogValueParser implements MethodInterceptor, Serializable, SmartInitializingSingleton {
    ...
    private Object execute(MethodInvocation invoker, Object target, Method method, Object[] args) throws Throwable {
        ...
        // 差异日志分三段执行：业务前解析函数、执行业务方法、业务后组装并保存日志。
    ...
    private void recordExecute(MethodExecute methodExecute, Map<String, String> functionNameAndReturnMap,
        ...
        for (DiffLogOps ops : logOps) {
            try {
                ...
                // joinTransaction=true 时，日志失败跟随业务事务回滚；否则只记录错误，不影响主流程。
                if (joinTransaction) {
                    throw t;
                }
            }
    ...
    private void successRecordExecute(MethodExecute methodExecute, Map<String, String> functionNameAndReturnMap,
        ...
        // successCondition 只在业务方法成功执行后生效，用于把“业务成功但条件不满足”的场景记为失败模板。
        String action;
        boolean flag = true;
    ...
    private boolean exitsCondition(MethodExecute methodExecute,
        ...
        if (StrUtil.isNotBlank(ops.getCondition())) {
            // condition 是总开关，解析结果为 false 时整条日志配置直接跳过。
    ...
    private void saveLog(Method method, boolean flag, DiffLogOps ops,
        ...
        if (StrUtil.isBlank(expressions.get(description))
            ...
            // 模板没有解析出有效内容时不落库，避免把原始 SpEL 表达式写进审计日志。
            return;
        }
