...
import com.microservice.framework.boot.log.event.AccessLogEvent;
...
import com.microservice.framework.commons.NativeUserAgent;
import com.microservice.framework.commons.RegionUtils;
...
public class AccessLogAspect {
    ...
    private static final Set<String> SENSITIVE_FIELD_NAMES = Set.of(
            "password", "passwd", "clientsecret", "accesstoken", "refreshtoken",
            "authorization", "apikey", "secret", "privatekey", "token");
     ...
     * 2. 或者 com.microservice 包下的 Controller 方法
     */
    @Pointcut("@annotation(com.microservice.framework.commons.annotation.log.AccessLog) || execution(* com.microservice..*controller..*Controller.*(..))")
    ...
    private Object handleAccessLog(ProceedingJoinPoint joinPoint, AccessLog accessLog) throws Throwable {
        ...
        try {
            ...
            if (accessLog.response()) {
                logInfo.setResponse(truncate(redactSensitiveJsonFields(JacksonUtils.toJson(ret)), MAX_LENGTH));
            }
            return ret;
        } catch (Throwable e) {
            ex = e;
            ...
            List<String> sensitiveValues = getSensitiveValues(request);
            sanitizeAccessLogInfo(logInfo, sensitiveValues);
    ...
    private String getSafeArgs(boolean recordRequest, Object[] args, HttpServletRequest request) {
        ...
        try {
            ...
            List<Object> arguments = Arrays.stream(args)
                    ...
                    .map(arg -> sensitiveValues.contains(arg) ? REDACTED : arg)
                    .collect(Collectors.toList());
            return redactSensitiveValues(redactSensitiveJsonFields(JacksonUtils.toJson(arguments)), sensitiveValues);
        } catch (Exception e) {
            return "Args serialization failed";
        }
    }

    private List<String> getSensitiveValues(HttpServletRequest request) {
        if (request == null) {
            return Collections.emptyList();
        }
        Set<String> sensitiveValues = new LinkedHashSet<>();
        addSensitiveValue(sensitiveValues, request.getHeader(HttpHeaders.AUTHORIZATION));
        String configuredTokenKey = accessLogProperties.getToken();
        if (StringUtils.hasText(configuredTokenKey)) {
            addSensitiveValue(sensitiveValues, request.getHeader(configuredTokenKey));
            addSensitiveValue(sensitiveValues, request.getParameter(configuredTokenKey));
            addSensitiveValue(sensitiveValues, request.getAttribute(configuredTokenKey));
        }
        return sensitiveValues.stream().sorted(Comparator.comparingInt(String::length).reversed()).toList();
    }

    private void addSensitiveValue(Set<String> sensitiveValues, Object value) {
        if (value != null && StringUtils.hasText(value.toString())) {
            sensitiveValues.add(value.toString());
        }
    }
    ...
    private String formatDebugLog(JoinPoint joinPoint, String descStr, Object ret, long executionTime, HttpServletRequest request) {
        ...
        String methodParam = serializeDebugArguments(joinPoint, sensitiveValues);
        ...
        redactConfiguredTokenParameter(methodParamMap);
        ...
        String authorization = StringUtils.hasText(request.getHeader(HttpHeaders.AUTHORIZATION)) ? REDACTED : null;
        ...
        return redactSensitiveValues(sb.toString(), sensitiveValues);
    }
    ...
    private String serializeDebugArguments(JoinPoint joinPoint, List<String> sensitiveValues) {
        List<Object> arguments = Arrays.stream(joinPoint.getArgs())
                .filter(arg -> !(arg instanceof ServletRequest || arg instanceof ServletResponse || arg instanceof MultipartFile))
                .map(arg -> sensitiveValues.contains(arg) ? REDACTED : arg)
                .collect(Collectors.toList());
        return truncate(redactSensitiveValues(safeJsonSerialize(arguments), sensitiveValues), MAX_LENGTH);
    }
    ...
    private void redactConfiguredTokenParameter(Map<String, Object> parameters) {
        if (parameters.isEmpty() || !StringUtils.hasText(accessLogProperties.getToken())) {
            return;
        }
        parameters.replaceAll((key, value) -> key.equalsIgnoreCase(accessLogProperties.getToken()) ? REDACTED : value);
    }

    private void sanitizeAccessLogInfo(AccessLogInfo logInfo, List<String> sensitiveValues) {
        logInfo.setToken(null);
        logInfo.setRequest(redactSensitiveJsonFields(redactSensitiveValues(logInfo.getRequest(), sensitiveValues)));
        logInfo.setResponse(redactSensitiveJsonFields(redactSensitiveValues(logInfo.getResponse(), sensitiveValues)));
        logInfo.setMessage(redactSensitiveValues(logInfo.getMessage(), sensitiveValues));
    }

    private String redactSensitiveValues(String value, List<String> sensitiveValues) {
        if (!StringUtils.hasText(value) || sensitiveValues.isEmpty()) {
            return value;
        }
        String redactedValue = value;
        for (String sensitiveValue : sensitiveValues) {
            redactedValue = redactedValue.replace(sensitiveValue, REDACTED);
        }
    ...
    private String safeJsonSerialize(Object obj) {
        ...
        try {
            return redactSensitiveJsonFields(JacksonUtils.toJson(obj));
        } catch (Exception e) {
    ...
    /**
     * 按字段名递归脱敏 JSON，避免登录密码和签发令牌在值尚未知时进入日志。
     */
    private String redactSensitiveJsonFields(String value) {
        if (!StringUtils.hasText(value)) {
            return value;
        }
        try {
            JsonNode root = JacksonUtils.toJsonNode(value);
            redactSensitiveJsonNode(root);
    ...
    private void redactSensitiveJsonNode(JsonNode node) {
        if (node instanceof ObjectNode objectNode) {
            ...
            for (String fieldName : fieldNames) {
                if (isSensitiveField(fieldName)) {
                    ...
                    redactSensitiveJsonNode(objectNode.get(fieldName));
                }
            }
            return;
        }
        if (node != null && node.isArray()) {
            node.forEach(this::redactSensitiveJsonNode);
        }
    }
