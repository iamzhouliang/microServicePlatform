...
public class EmailMessageNotifyStrategy implements MessageNotifyStrategy {
    ...
    public JavaMailSenderImpl getMailSender(MessageChannel channel) {
        ...
        if (factory != null && factory.getSetting() == setting) {
            log.debug("检测到邮件配置未发生变更, 当前采用已创建的发送器, channelId={}", channel.getId());
        ...
        log.info("邮件发送器初始化完成, channelId={}, host={}, port={}, protocol={}", channel.getId(), setting.getHost(), setting.getPort(), setting.getProtocol());
        return mailSender;
    }
    ...
    public void handler(MessageChannel channel, MessageNotify notify) {
        ...
        log.debug("邮箱消息准备发送, channelId={}, host={}, subscribe={}, notify={}", channel.getId(), mailSender.getHost(), notify.getSubscribe(),
                JacksonUtils.toJson(notify));
        ...
        log.info("邮箱消息发送完成, channelId={}, userId={}, subscribe={}", channel.getId(), notify.getUserId(), notify.getSubscribe());
    }
