...
import com.microservice.framework.commons.annotation.remote.RemoteResult;
...
import com.microservice.framework.db.mybatisplus.datascope.core.DataScope;
...
import com.microservice.framework.log.diff.core.annotation.DiffLog;
import com.microservice.framework.log.diff.core.context.DiffLogContext;
...
import com.microservice.platform.iam.auth.domain.entity.LoginLog;
import com.microservice.platform.iam.auth.support.domain.UserTenantAuthentication;
import com.microservice.platform.iam.base.domain.dto.req.ChangeUserInfoReq;
import com.microservice.platform.iam.system.domain.dto.req.UserOnlinePageReq;
import com.microservice.platform.iam.system.domain.dto.req.UserPageReq;
...
import com.microservice.platform.iam.system.domain.dto.req.UserUpdateReq;
import com.microservice.platform.iam.system.domain.dto.resp.UserPageResp;
import com.microservice.platform.iam.system.domain.entity.Resource;
...
public class UserServiceImpl extends SuperServiceImpl<UserMapper, User> implements UserService {
    ...
    public void changePassword(Long userId, String orgPassword, String newPassword) {
        Optional.ofNullable(this.baseMapper.selectById(userId)).orElseThrow(() -> CheckedException.notFound("用户不存在"));
    ...
    public void delete(Long id) {
        ...
        userRoleService.deleteByUsers(List.of(id));
    }
    ...
    public void updateProfile(UserProfileUpdateReq req) {
        ...
        User bean = User.builder().id(userId).email(req.getEmail()).mobile(req.getMobile())
                .nickname(req.getNickname()).birthday(req.getBirthday())
                .description(req.getDescription()).build();
        if (this.baseMapper.updateById(bean) <= 0) {
            throw CheckedException.notFound("用户不存在");
        }
        UserInfoDetails userInfo = (UserInfoDetails) context.getContext();
        if (userInfo == null) {
            return;
        }
        ...
        userInfo.setBirthday(req.getBirthday());
        userInfo.setDescription(req.getDescription());
        StpUtil.getTokenSession().set(extProperties.getServer().getTokenInfoKey(), userInfo);
    }

    @Override
    public void updatePassword(UserPasswordUpdateReq req) {
        if (!StrUtil.equals(req.getNewPassword(), req.getConfirmPassword())) {
            throw CheckedException.badRequest("新密码与确认密码不一致");
        }
        Long userId = context.userId();
        User user = Optional.ofNullable(this.baseMapper.selectById(userId))
                .orElseThrow(() -> CheckedException.notFound("用户不存在"));
        if (!PasswordEncoderHelper.matches(req.getCurrentPassword(), user.getPassword())) {
            throw CheckedException.badRequest("原始密码错误");
        }
        User bean = User.builder().id(userId).password(PasswordEncoderHelper.encode(req.getNewPassword())).build();
        if (this.baseMapper.updateById(bean) <= 0) {
            throw CheckedException.notFound("用户不存在");
        }
