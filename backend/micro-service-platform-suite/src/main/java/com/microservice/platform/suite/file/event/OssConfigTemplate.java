...
package com.microservice.platform.suite.file.event;
...
import com.microservice.platform.suite.file.domain.constants.StorageConstants;
import com.microservice.platform.suite.file.domain.entity.OssConfig;
import com.microservice.platform.suite.file.repository.OssConfigMapper;
...
import static com.microservice.platform.suite.file.domain.constants.StorageConstants.STORAGE_SETTING_DEFAULT_SETTING;
...
public class OssConfigTemplate implements ApplicationRunner {
    ...
    public void publish(OssConfig setting, int eventType) {
        // OSS 配置包含访问密钥，日志中只记录排查事件所需的非敏感元数据。
        log.info("发布存储配置事件，租户ID：{}，平台：{}，事件类型：{}", setting.getTenantId(), setting.getPlatform(), eventType);
        ...
        // 监听端按标准 JSON 解析消息，主题广播必须使用字符串序列化，避免对象类型元数据破坏跨实例反序列化。
        stringRedisTemplate.convertAndSend(StorageConstants.STORAGE_CONFIG_EVENT_TOPIC, JacksonUtils.toJson(event));
