...
const getDefaultPwaOptions = (name: string): Partial<PwaPluginOptions> => ({
  manifest: {
    description: `${name} is a modern admin dashboard template based on Vue 3. `,
    icons: [
      {
        sizes: '192x192',
 ...
 *
 * defaultProvider 通过环境变量 `IMPORTMAP_PROVIDER` 配置（默认 esm.sh）；
 * 国内/内网部署可设为 `cncdn.jsdelivr.net` 或私有 CDN。
 */
const defaultImportmapOptions: ImportmapPluginOptions = {
  // 通过 Importmap CDN 方式引入,
  // 目前只有esm.sh源兼容性好一点，jspm.io对于 esm 入口要求高
  defaultProvider: process.env.IMPORTMAP_PROVIDER || 'esm.sh',
  importmap: [
    { name: 'vue' },
    { name: 'pinia' },
    { name: 'vue-router' },
