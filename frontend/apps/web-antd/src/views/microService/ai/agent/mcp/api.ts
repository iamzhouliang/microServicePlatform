...
export const AddObj = (data: McpServerConfigSaveReq) =>
  defHttp.post('/ai/mcp-server/create', data);

// 修改配置
export const UpdateObj = (id: number, data: McpServerConfigSaveReq) =>
  defHttp.put(`/ai/mcp-server/${id}/modify`, data);
...
export const RefreshConnection = (id: number) =>
  defHttp.request(`/ai/mcp-server/${id}/refresh`, {
    method: 'PATCH',
  });
...
export const ToggleStatus = (id: number, status: boolean) =>
  defHttp.request(`/ai/mcp-server/${id}/status`, {
    method: 'PATCH',
    params: { status },
  });
