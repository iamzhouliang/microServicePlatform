...
interface TabbarState {
   ...
   * @zh_CN 当前打开的标签页列表
   *
   * 元素类型声明为 TabDefinition，但 Pinia store 内部 this.tabs 在 actions 里被 TS
   * 推断为 vue-router RouteLocationNormalizedLoaded 类型（与 TabDefinition 字段重叠但不完全兼容），
   * 联合 any 是无奈——unknown 会让传给 TabDefinition 函数报错。
   */
